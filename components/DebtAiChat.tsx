...
import { Bot, CheckCircle2, Clipboard, Download, ExternalLink, Link2, LoaderCircle, LogOut, Send, Sparkles, UserRound, X } from "lucide-react";
    ...
    <div className="ai-drawer-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <aside className="ai-drawer" aria-label="Hỏi AI về công nợ">
        ...
        <div className="ai-data-bar">
          <div><strong>Dữ liệu trực tiếp từ Supabase</strong><span>Codex chỉ lấy phần liên quan đến câu hỏi</span></div>
        ...
        {messages.length === 1 && (
          <div className="ai-suggestions">
            {suggestions.map((suggestion) => <button type="button" key={suggestion} disabled={!codexStatus?.authenticated && !codexStatus?.fallback_available} onClick={() => void submit(undefined, suggestion)}>{suggestion}</button>)}
          </div>
        )}
        ...
        <form className="ai-composer" onSubmit={(event) => void submit(event)}>
          ...
          <div>
            ...
            <button className="primary-button" type="submit" disabled={!input.trim() || loading || (!codexStatus?.authenticated && !codexStatus?.fallback_available)} aria-label="Gửi câu hỏi"><Send size={18} /></button>
          </div>
