...
import { formatDate, formatOverdueDays, integer, money } from "@/lib/format";
...
export function DataTable({ kind, rows, title, onEdit, onDelete, onPay, compact = false, hideToolbar = false }: Props) {
  ...
  const actions = Boolean(onEdit || onDelete || onPay);

  return (
    <section className="table-card">
      {!hideToolbar && (
        <div className="table-toolbar">
          <div><strong>{title || tableTitle(kind)}</strong><span>{rows.length.toLocaleString("vi-VN")} bản ghi</span></div>
        </div>
      )}
            ...
            {kind === "debts"
              ? (visible as DebtRow[]).map((row) => (
                <tr key={row.id} className={row.status === "overdue" ? "overdue-row" : ""}>
                  ...
                  <td className={row.status === "overdue" ? "overdue-date" : ""}>{formatDate(row.due_date)}</td>
                  <td className={`number-cell ${row.status === "overdue" ? "overdue-days" : ""}`}>{formatOverdueDays(row.due_date, row.status)}</td>
                  ...
                  {actions && debtActionCell(row, onPay, onEdit, onDelete)}
                </tr>
              ))
              : kind === "overview"
                ? (visible as DebtRow[]).map((row) => (
              ...
              : kind === "payments"
                ? (visible as PaymentRow[]).map((row) => (
                  <tr key={row.id}>
                    <td><div className="customer-cell"><strong>{row.customer_name || row.debt?.customer?.name || "Không rõ"}</strong><small>{row.customer_code || row.phone || "Chưa có mã KH"}</small></div></td>
                    <td>{formatDate(row.debt_order_date || row.debt?.order_date)}</td>
...
function renderHeader(kind: Props["kind"], actions: boolean) {
  if (kind === "payments") return <tr><th>Trả cho khách</th><th>Ngày nợ</th><th>Ngày trả</th><th className="number-cell">Số tiền trả</th><th>NV kinh doanh</th><th>NV giao hàng</th><th>Ghi chú</th>{actions && <th />}</tr>;
  ...
  if (kind === "debts") return <tr><th>Khách hàng</th><th>Ngày nợ</th><th>Hạn trả</th><th className="number-cell">Số ngày quá hạn</th><th>NV kinh doanh</th><th>NV giao hàng</th><th className="number-cell">Tiền nợ</th><th className="number-cell">Đã trả</th><th className="number-cell">Còn lại</th><th>Trạng thái</th>{actions && <th />}</tr>;
...
function debtActionCell(row: DebtRow, onPay?: (row: DebtRow) => void, onEdit?: (row: Row) => void, onDelete?: (id: string) => void) {
  return (
    <td className="actions-cell debts-actions-cell">
      {onPay && row.remaining_amount > 0 && (
        <button className="pay-button" type="button" onClick={() => onPay(row)}>Thanh toán</button>
      )}
      {onEdit && <button className="icon-button" type="button" onClick={() => onEdit(row)} title="Sửa"><Pencil size={16} /></button>}
      {onDelete && <button className="icon-button danger" type="button" onClick={() => onDelete(row.id)} title="Xoá"><Trash2 size={16} /></button>}
    </td>
  );
}
